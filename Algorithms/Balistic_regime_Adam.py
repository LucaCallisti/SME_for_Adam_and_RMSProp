...
class Adam_SDE_2order_balistic_regime(SDE_basic):
    """
    Implements the 2nd order Adam optimizer in the ballistic regime as an SDE.
    """
    @torch.no_grad()
    def __init__(self, eta, c, function, All_time, regularizer, epsilon = 1e-6, sigma_value=0.5, constant_noise = True, Verbose = True):
        """
        Initialize the Adam SDE for the ballistic regime.
        Args:
            eta: Learning rate.
...
class Adam_SDE_1order_balistic_regime(Adam_SDE_2order_balistic_regime):
    """
    Implements the 1st order Adam optimizer in the ballistic regime as an SDE.
    """
    @torch.no_grad()
    def f(self, t, x):
        """
...
class Adam_deterministic(Adam_SDE_2order_balistic_regime):
    """
    Implements the deterministic version of the Adam optimizer in the ballistic regime.
    """
    @torch.no_grad()
    def f(self, t, x):  
        """
...
def Discrete_Adam_balistic_regime(funz, noise, tau, beta, c, num_steps, x_0, skip, epsilon = 1e-6, verbose = False, loss_bool = True):
    """
    Discrete implementation of the Adam optimizer in the ballistic regime.
    Args:
