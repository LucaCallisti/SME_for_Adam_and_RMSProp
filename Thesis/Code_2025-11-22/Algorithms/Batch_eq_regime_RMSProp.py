...
class RMSprop_SDE_2order_batch_eq_regime(SDE_basic):
    ...
    def b_1_theta(self, denom, v_reg, v_reg_grad):
        ...
        additional_term = 2 * torch.bmm( torch.bmm(torch.diag_embed(denom), self.term_b_1_theta), denom.unsqueeze(2)**2)
...
def Discrete_RMProp_batch_eq_regime(funz, noise, tau, beta, c, num_steps, x_0, skip,  epsilon = 1e-6, verbose = False, loss_bool = True):
    ...
    for step in range(num_steps-1):
        ...
        g = funz.noisy_grad_batcheq(x, gamma, tau)
        path_v[:, step+1] = beta * v + tau**2 * c * g**2
        path_x[:, step+1] = x - tau * g / (torch.sqrt(v) + epsilon)
