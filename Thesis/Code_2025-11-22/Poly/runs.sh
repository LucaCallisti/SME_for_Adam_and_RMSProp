...
do
    ...
    do
        ...
        python -m Poly.main --regime batch_equivalent --optimizer RMSProp --sigma $sigma_Beq --batch-size-simulation $((k*12)) --noise_level $nl --tau-list $tau --name-project "Poly_scaling_rule_batch_tau" &

        # 2. Batch Equivalent - Adam
        echo "Running: Batch Equivalent | Adam | Sigma -1"
        python -m Poly.main --regime batch_equivalent --optimizer Adam --sigma $sigma_Beq --batch-size-simulation $((k*12)) --noise_level $nl --tau-list $tau --name-project "Poly_scaling_rule_batch_tau" &
