...
def process_results(tensor: torch.Tensor, checkpoints: torch.Tensor, variable : str, Norm_bool : bool = True) -> torch.Tensor:
    """
    Normalize tensor and compute mean and std across batches, and extract distributions at checkpoints.
    Args:
        ...
        Norm_bool: Whether to compute norm across last dimension.
    Returns:
    ...
    result = {
        ...
        variable + '_last_step': input_
    }
    return result



def load_and_preprocess_data(dataset) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Load and preprocess datasets (Housing, BreastCancer, MNIST, CIFAR10).
    Args:
        dataset: Name of the dataset to load.
    Returns:
        Tuple of (X_train, y_train) as torch tensors.
    """
...
def get_parameters(model_type):
    ...
    Args:
        model_type: String specifying the model type.
    Returns:
        Tuple of (tau, sigma, final_time, num_runs, batch_size, c, c1, c2).
    """
