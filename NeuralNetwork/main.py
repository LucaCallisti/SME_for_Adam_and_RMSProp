...
def parse_arguments() -> argparse.Namespace:
    ...
    output_group.add_argument('--results-dir', type=str, default='./Results_NeuralNetwork_models/', help='Base directory for saving results')
...
def run_sde_simulations(
    ...
    """
    Run SDE (Stochastic Differential Equation) simulations for neural network training.
    Args:
...
def run_discrete_simulations(
    ...
    """
    Run discrete optimizer simulations for neural network training.
    Args:
        model_factory: Callable that returns a model instance.
        optimizer: Optimizer name.
        regime_funcs: Dictionary of regime-specific functions.
...
def main():
    """
    Main execution function for neural network SDE/discrete training experiments.
    """
    ...
    if args.model == 'ShallowNN':
        ...
        if args.optimizer == 'RMSProp' and args.regime == 'batch_equivalent':
            Batch_for_hessian = 75
        else:
            Batch_for_hessian = 75
        def model_factory():
            return ResNet(device=args.device, Batch_for_hessian=Batch_for_hessian)
