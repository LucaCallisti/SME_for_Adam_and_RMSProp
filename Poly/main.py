...
import torchsde
import math
from Poly.Utils import set_seed, processing_outputs_caller, log_to_wandb, calculate_loss
from Poly.poly import Poly_with_additional_noise
...
def parse_arguments() -> argparse.Namespace:
    ...
    train_group.add_argument('--final-time', type=float, default=4.0, help='Final time for SDE integration')
    ...
    output_group.add_argument('--results-dir', type=str, default='./Results_Polynomial/', help='Base directory for saving results')
...
def run_sde_simulations(
    ...
    """
    Run SDE (Stochastic Differential Equation) simulations for function approximation.
    Args:
        poly: Polynomial function object.
        optimizer: Optimizer name.
        regime_funcs: Dictionary of regime-specific functions.
        which_approximation: Which SDE approximation to use.
        initial_points: Initial parameter tensor.
        tau: Step size.
        c: Coefficient(s).
    ...
    if optimizer == 'Adam':
        dim_result = dim_weights * 3 
    elif optimizer == 'RMSProp':
        dim_result = dim_weights * 2  
    ...
    t1 = time.time()
    loss = calculate_loss(poly, runs[:, :, :dim_weights])
...
def run_discrete_simulations(
    ...
    """
    Run discrete optimizer simulations for function approximation.
    Args:
        poly: Polynomial function object.
        optimizer: Optimizer name.
        regime_funcs: Dictionary of regime-specific functions.
    ...
    loss = calculate_loss(poly, discrete_runs[:, :, :dim_weights])
    t1 = time.time()
...
def _run_1st_order_balistic(
    ...
    Args:
        ...
        verbose: Verbosity flag.
    Returns:
        Tuple of results for stochastic and deterministic simulations.
    """
...
def run_experiment_configuration(
    ...
    Args:
        ...
        tau: Step size.
        sigma_value: Noise scale.
        points: List of function parameters.
        final_time: Final simulation time.
        checkpoints: Checkpoint tensor.
        initial_points_before_disc: Initial parameter tensor (optional).
        epsilon: Regularization parameter.
    Returns:
        None
    """
