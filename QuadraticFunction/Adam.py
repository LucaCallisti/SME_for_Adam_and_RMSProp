...
import Algorithms.Balistic_regime_Adam as BR_Adam
import Algorithms.Batch_eq_regime_Adam as BER_Adam
import os
import time
...
def Dynamic_simulation(fun, eta, c, beta, skip_initial_point, step, y0, final_time, option, regularizer, Verbose = False, epsilon = 1e-3, dataset = None):
    """
    Simulates the dynamics of the Adam optimizer under various regimes.
    ...
    Args:
        ...
        eta: Learning rate.
        c: Tuple of constants for the optimizer.
        beta: Tuple of beta parameters for Adam.
    ...
    regularizer.set_costant( eta *torch.ones_like(y0[:, 4:]) ) 
    if option == 'Adam_dyscrete_balistic_regime':
        return BR_Adam.Discrete_Adam_balistic_regime(fun, dataset, eta, beta, c, step, y0, skip = skip_initial_point, epsilon=epsilon, loss_bool = False)[:, skip_initial_point:, :]
...
def N_simulations(fun, regularizer, y0, eta, beta, c, final_time, run, batch_size, opt_discr, opt_cont_1, opt_cont_2, opt_cont_3=None, epsilon = 1e-6, dataset =None):
    ...
    Run_d_meanVar_m = utils.OnlineMeanVar(step-skip_initial_point, device=torch.device("cpu"))
    Run_c_1_meanVar_m = utils.OnlineMeanVar(step-skip_initial_point, device=torch.device("cpu"))
    Run_c_2_meanVar_m = utils.OnlineMeanVar(step-skip_initial_point, device=torch.device("cpu"))
    ...
    if opt_cont_3 != None:
        ...
        Run_c_3_meanVar_m = utils.OnlineMeanVar(step-skip_initial_point, device=torch.device("cpu"))
        Run_c_3_meanVar_v = utils.OnlineMeanVar(step-skip_initial_point, device=torch.device("cpu"))

    print(f'Starting simulation with eta: {eta:.3f}, beta: {beta[0]:.3f}_{beta[1]:.3f}, c: {c[0]:.3f}_{c[1]:.3f}')
    ...
    for i in range(run_SDE_2):
        ...
        Run_d_meanVar_m.update(g_function(y_discr)[:, :, 1])
        Run_d_meanVar_v.update(g_function(y_discr)[:, :, 2])
    ...
    for i in range(run_SDE_2):
        ...
        Run_c_2_meanVar_m.update(g_function(y_cont_2)[:, :, 1])
        Run_c_2_meanVar_v.update(g_function(y_cont_2)[:, :, 2])
    ...
    for i in range(run_SDE_1):
        ...
        Run_c_1_meanVar_m.update(g_function(y_cont_1)[:, :, 1])
        Run_c_1_meanVar_v.update(g_function(y_cont_1)[:, :, 2])
    ...
    if opt_cont_3 != None:
        ...
        for i in range(run_SDE_1):
            ...
            Run_c_3_meanVar_m.update(g_function(y_cont_3)[:, :, 1])
            Run_c_3_meanVar_v.update(g_function(y_cont_3)[:, :, 2])

    del y_discr, y_cont_1, y_cont_2
    if opt_cont_3 != None: del y_cont_3
    torch.cuda.empty_cache()

    Run_d_mean = torch.stack((Run_d_meanVar_theta.get_mean(), Run_d_meanVar_m.get_mean(), Run_d_meanVar_v.get_mean()), dim=1)
    Run_c_1_mean = torch.stack((Run_c_1_meanVar_theta.get_mean(), Run_c_1_meanVar_m.get_mean(), Run_c_1_meanVar_v.get_mean()), dim=1)
    Run_c_2_mean = torch.stack((Run_c_2_meanVar_theta.get_mean(), Run_c_2_meanVar_m.get_mean(), Run_c_2_meanVar_v.get_mean()), dim=1)

    if opt_cont_3 != None:
        Run_c_3_mean = torch.stack((Run_c_3_meanVar_theta.get_mean(), Run_c_3_meanVar_m.get_mean(), Run_c_3_meanVar_v.get_mean()), dim=1)
        return Run_d_mean, Run_c_1_mean, Run_c_2_mean, Run_c_3_mean
    return Run_d_mean, Run_c_1_mean, Run_c_2_mean
def g_function(x):
    """
    Splits the input tensor into theta, m, and v components and computes their norms.
    Args:
        x: Input tensor of shape (batch, time, features).
    Returns:
        Concatenated tensor of norms for theta, m, and v.
    """
    split = x.shape[-1]//3
    theta = x[:, :, :split]
    m = x[:, :, split:2*split]
    ...
    return torch.concat(( aux(theta, split).unsqueeze(2), aux(m, split).unsqueeze(2), aux(v, split).unsqueeze(2)  ), dim = 2)


def Simulation_sde(opt_discr, opt_cont_1, opt_cont_2, opt_cont_3 = None, A=None, y0=None, beta_1_fun=None, beta_2_fun=None, path_folder = None, path_folder_for_opt_3=None, dataset=None):
    """
    Orchestrates the simulation of Adam optimizer for different regimes and learning rates.
    ...
    Args:
        ...
        y0: Initial state tensor.
        beta_1_fun: Function to compute beta_1 given eta.
        beta_2_fun: Function to compute beta_2 given eta.
    ...
    if opt_cont_3 != None:
        data_theta_final_opt_3, data_theta_max_opt_3, data_m_final_opt_3, data_m_max_opt_3, data_v_final_opt_3, data_v_max_opt_3, Res_opt_3 = {}, {}, {}, {}, {}, {}, {}
    Res['Matrix'] = fun.A
    ...
    regularizer = BR_Adam.Regularizer_ReLu()
    start_tot = time.time()
    for i in [1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]:
        ...
        power = '2'
        eta = 2**(-i)
        beta_1 = beta_1_fun.get_beta(eta)
        beta_2 = beta_2_fun.get_beta(eta)
        beta = (beta_1, beta_2)
        c_1 = beta_1_fun.get_c(eta)
        c_2 = beta_2_fun.get_c(eta)
        c = (c_1, c_2)

        print(f"\n Starting simulation with eta = {eta:.3f}, beta_1 = {beta_1:.3f}, beta_2 = {beta_2:.3f}, c_1 = {c_1:.3f}, c_2 = {c_2:.3f} \n")
        ...
        data_final_list = [data_theta_final, data_m_final, data_v_final]
        data_max_list = [data_theta_max, data_m_max, data_v_max]
        ...
        data_theta_final, data_m_final, data_v_final = data_final_list
        data_theta_max, data_m_max, data_v_max = data_max_list
        
        if opt_cont_3 != None:
            data_final_list_opt_3 = [data_theta_final_opt_3, data_m_final_opt_3, data_v_final_opt_3]
            data_max_list_opt_3 = [data_theta_max_opt_3, data_m_max_opt_3, data_v_max_opt_3]
            ...
            data_theta_final_opt_3, data_m_final_opt_3, data_v_final_opt_3 = data_final_list_opt_3
            data_theta_max_opt_3, data_m_max_opt_3, data_v_max_opt_3 = data_max_list_opt_3
...
def multiple_simulations(Main_folder=None, opt_discr=None, opt_cont_1=None, opt_cont_2=None, opt_cont_3 = None, folder_opt_3 = None, dataset=None, initial_point=[10., 5.], seed = 0):
    ...
    fixed_values_c = [(1, 0.5)]
    for (c_1, c_2) in fixed_values_c:
        c_1_fun = utils.fixed_c(c_1)
        c_2_fun = utils.fixed_c(c_2)
        folder_path = os.path.join(Main_folder, f"SDE1_1approx_SDE2_2approx_c_{c_1}_{c_2}")
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        
        if opt_cont_3 != None:
            folder_opt_3_joined = os.path.join(folder_opt_3, f"SDE1_1approx_SDE2_2approx_c_{c_1}_{c_2}")
            if not os.path.exists(folder_opt_3_joined):
                os.makedirs(folder_opt_3_joined)
        else:
            folder_opt_3_joined = None
        Simulation_sde(opt_discr, opt_cont_1, opt_cont_2, opt_cont_3=opt_cont_3, A=A, y0=y0, beta_1_fun=c_1_fun, beta_2_fun = c_2_fun, path_folder=folder_path, path_folder_for_opt_3 = folder_opt_3_joined, dataset=dataset)
    

def Adam_balistic_regime():
    """
    Runs the Adam optimizer simulation in the balistic regime and saves the results.
    Returns:
        None
    """
    ...
    opt_cont_1 = 'Adam_SDE_1order_balistic_regime'
    opt_cont_2 = 'Adam_SDE_2order_balistic_regime'
    opt_cont_3 = 'Adam_Approx_1order_balistic_regime'
...
def Adam_batch_equivalent_regime():
    """
    Runs the Adam optimizer simulation in the batch equivalent regime and saves the results.
    Returns:
        None
    """
    Main_folder = './Results_QuadraticFunction'
...
if __name__ == "__main__":
    Adam_batch_equivalent_regime()
    Adam_balistic_regime()
